...
import static org.ubl.scb.JAI.getImageDimensions;
...
public class VorlesungImpl {
    ...
    public List<String> getFileNames() {
        ...
        try (Stream<Path> paths = Files.walk(Paths.get(imageSourceDir))
                .filter(Files::isRegularFile)) {
            paths.forEach(p -> {
                final String name = FilenameUtils.removeExtension(p.getFileName()
                        .toString());
                names.add(name);
            });
            return names;
